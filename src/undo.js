...
function addRawUndo(data, pageid) {
	...
	var menuopt = '<li><input type="button" value="Undo Requirements" data-theme="g" id="buttonPopUndo" >\n';

	data = data.slice(0, startfunc) + menuopt + '\n' + data.slice(startfunc);

	var startfunc = data.indexOf('<div id="footer"');
	newdata = `<div data-role="popup" id="undoRequirementsPopup" data-dismissible="false" data-theme="d" data-history="false"><ul data-role="listview" data-inset="true" style="min-width: 600px;" data-theme="d" ><li data-role="divider" data-theme="e">Undo or Reset Requirements:</li>			<li data-theme="d">
				<div class="ui-grid-b ui-responsive">
					<div class="ui-block-a">
						<div>	
							<div class="clearRight"></div>
							<div style="margin-top: 0; " class="ui-icon-alt">
								<fieldset data-role="controlgroup">
									<legend class="text-orange">
										<strong>Choose what marking you want to undo:</strong>
									</legend>
									<select name="UndoTypeID" id="undoTypeID" data-theme="d">
										<option value="">choose one...</option>
											<option value="1" >Marked Completed</option>
											<option value="2" >Marked Approved</option>		
											<option value="3" >Marked Either Completed Or Approved</option>	
									</select>
								</fieldset>
							</div>
						</div>
					...
					<div class="ui-block-b">
						<div>	
							...
							<div style="margin-top: 0; " class="ui-icon-alt">
								<fieldset data-role="controlgroup">
									<legend class="text-orange">
										<strong>Choose date marked you want to undo:</strong>
									</legend>
									<select name="UndoDateID" id="undoDateID" data-theme="d">
										<option value="">choose one...</option>
					...
					<div class="ui-block-c">
						<div>	
							...
							<div style="margin-top: 0; " class="ui-icon-alt">
								<fieldset data-role="controlgroup">
									<legend class="text-orange">
										<strong>Choose who's marking you want to undo:</strong>
									</legend>
									<select name="UndoWhoID" id="undoWhoID" data-theme="d">
										<option value="">choose one...</option>
			...
			<li>
			<input type="button" value="Undo Now" data-theme="g" id="buttonUndo" >
			<input type="button" value="Cancel" data-theme="g" id="buttonUndoCancel" >
			</li>
			</ul>
			</div>`;
...
function procUndo(pageid) {
	var pageASP;
	var li;
	if (document.URL.includes('adventure.asp')) {
		pageASP = 'adventurerequirement';
		li = 'adventureCompletedLI';
	}
	else if (document.URL.includes('meritbadge.asp')) {
		pageASP = 'meritbadgerequirement';
		li = 'mbCompletedLI';
	}
	else if (document.URL.includes('award.asp')) {
...
function postUndos(removeReqList, pageid) {
	...
	if (url == '') {
		$('#undoRequirementsPopup', '#PageX').popup('close');
		$('#buttonPopUndo,#buttonUndoCancel,#buttonUndo', '#PageX').button('enable');
