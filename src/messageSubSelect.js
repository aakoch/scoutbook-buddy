...
function divContent(stype, subUnitList) {
    ...
    for (var i = 0; i < subUnitList.length; i++) {
        ...
        html += escape(subUnitList[i].name);
        html += '					</div>\n';
...
function createSubunitHtmlList(subUnitList) {
    return subUnitList.map((subunit) => {
        return `<li data-theme="d" class="subunit checkable " data-subunitid="${escape(subunit.id)}">
...
function startGetSubunits(unitID, subUnitScouts, preset) {
    ...
    var url = 'https://' + document.location.hostname.split('.')[0] + '.scoutbook.com/mobile/dashboard/admin/unit.asp?UnitID=' + escape(unitID);
    $.get(url)
        .done(function(data, textStatus, jqXHR) {
            ...
            var denpatrol;
            var name;
            $('a[href*="admin/denpatrol.asp?UnitID="]', data).each(function() {
                denpatrol = '';
            ...
            var html = createSubunitHtmlList(subUnitList);
            $('#denpatrolUL').append(html).listview('refresh');

            // add scripts
            subunitClick('');
            subunitmark();
            fetchPatrols(unitID, subUnitScouts, subUnitList, preset); // now that we have the subunits defined, iterate through them to capture scoutbook preselectes on the subunit
        })
        .fail(function(data, textStatus, jqXHR) {
            errStatusHandle(jqXHR.status, '', [], startGetSubunits, [unitID, subUnitScouts, preset]);
        });
}
...
function includeDisclaimer() {
    ...
    //if text is selected vs message, do not include
    if ($('input[name="MessageType"]').val() == 'email') {
