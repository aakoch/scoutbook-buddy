...
function getPayBalances(pageid, unitID) {
    ...
    $.mobile.loading('show', {
        theme: 'a',
        text: 'processing...this can take several minutes for large units',
        textonly: false
    });
    payTotals = [];
    var firstname;
...
function getParIdsFromConns(unitID, pageid, troop, type, initSendEmail, iunitID, ipageid) {
    ...
    xhttp.onreadystatechange = function() {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            setTimeout(function() {
                getParIdsFromConns(unitID, pageid, troop, type, initSendEmail, iunitID, ipageid);
            }, 200);
        }
    };
            ...
            if ($('#PastDueOnly', '#Page' + pageid).prop('checked') == false || parseFloat(payTotals[i].amt) < $('#DueAmt', '#Page' + pageid).val()) {
                ...
                if (skipNoPar == false) {
                    ...
                    var data = {
                        UnitID: unitid,
                        MessageType: 'email',
                        Subject: 'Scout Account Balance ' + due + 'Notice',
                        Body: '',
                        File: [{
                            filename: '',
                            filedata: ''
...
function procProfileForPay(unitID, pageid) {
    ...
    $.mobile.loading('show', {
        theme: 'a',
        text: 'preparing and sending notices.  This may take a long time for large units...',
        textonly: false
    });
    var utype;
    utype = "unit";
...
function getProfileforPayBal(thisScoutID, unitid, sPage, txtunit, type, pageid) {
    ...
    xhttp.onreadystatechange = function() {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            permCnt += 1;
            setTimeout(function() {
                getAccountForPayBal(unitid, sPage, txtunit, type, initSendEmail, unitid, pageid, pageid);
            }, 200);
        }
    };
