...
function deccu(pageid) {
	var UserStat={csvArray:[],ScoutbookUserID:'',Adult:'',Position:'',ScoutbookUnitID:'',Unit:'',District:'',DateLastLogin:'',nameArray:[['Scoutbook User ID','ScoutbookUserID'],['Adult','Adult'],['Position','Position'],['Scoutbook Unit ID','ScoutbookUnitID'],['Unit','Unit'],['District','District'],['Date Last Login','DateLastLogin']]};
	captureCouncilUsers(UserStat,pageid);
}

function captureCouncilUsers(UserStat,pageid) {

	$.mobile.loading('show', { theme: 'a', text: 'Getting Council Users Data', textonly: false });
...
function processStats(UserStat) {
	...
	for(var i=1; i<UserStat.csvArray.length;i++) {
		...
		if(lastunit!=UserStat.csvArray[i][UserStat.ScoutbookUnitID] ) {
			unitObj.push({recent:100,ScoutbookUnitID:UserStat.csvArray[i][UserStat.ScoutbookUnitID], Unit:UserStat.csvArray[i][UserStat.Unit], District:UserStat.csvArray[i][UserStat.District] });  
		}
...
function recentDate(indate) {
	if (indate=='') {
		//console.log(indate,'never logged in');
		return 100;
	}
	var dv=dateDiff(nowDate(),indate)/1000/60/60/24;
	
	//console.log(indate,parseInt(dv));
	//if (parseInt(dv) < numDays) {
		if(parseInt(dv) < 31) {
			return 30;
		}
		if(parseInt(dv) < 61) {
