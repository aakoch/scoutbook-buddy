...
chrome.runtime.onMessage.addListener(function(msgobj, sender) {
    ...
    } else if (ev.text == "showHelp") {
        chrome.tabs.create({
            url: "options.html"
        });
    } else if (ev.text == "XHRProxy_") {
        //evx.msg= JSON.stringify(ev.data);
        XHR_Proxy(ev);
    } else if (ev.text == "RestartSession") {
...
function isLocalLibraryCmd(cmd) {
    return ['ShowTimeoutWarning', 'ShowLoggedOffPopup', 'HidePopups'].includes(cmd);
}
...
function postMsg(ev, id) {
    ...
    logDebug('postMessage from background to ' + ev.hostx + ' is:' + ev.text);
    ...
    if (typeof(chrome.tabs) === 'object') {
        ...
        chrome.tabs.query({
            ...
            //dbconsole.log('have active tab - sending');	
            chrome.tabs.sendMessage(tabs[0].id, ev);
            logDebug('Sending to tab ' + tabs[0].id);
        });
    }
