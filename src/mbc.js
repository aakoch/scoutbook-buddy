...
function mbcreqqescript (){
	...
	$('.calendarIcon', '#PageX').on('click', function() {
		...
		function submitQuickForm() {
			...
			var evObj = {scoutname:'',scoutid: '',mbid: '',mbverid: '', mbreq: [],notes:'',unitid:''};
...
function connectNextMBC(counselorID,unitid,msg) {
	...
	if(sUIindex<0) {
		...
		setTimeout(function() {checkScoutConnection(counselorID,msg,unitid);},100);
	}	
}
...
function mbcqeinv() {
		...
		//Goes through each scout one at a time.
			...
			//If not connected sends invite				
				...
				setTimeout(function(){ checkScoutConnection(counselorID,msg,UnitID);},200);
...
function getMyPositions(document) {
			myPositions =[];
			var evObj ={unitID:'',position:'',mbs:[],unitName:'',myZip:''};			
	...
	xhttp.onreadystatechange = function() {
		...
		if (this.readyState == 4 && this.status == 200) {
			...
			for(var j=0;j<pmid.length;j++) {
				...
				evObj.mbLst.push({mbShortName:$('label[for="'+pmid[j]+'"]',this.response).text().trim(),mbid:pid[j],mbver:''});			
			}
...
function getMyProfileInfo(cb,cbArgObj,cberr,cberrarglst) {	
	...
	xhttp.onreadystatechange = function() {
		...
		if (this.readyState == 4 && this.status == 200) {
			...
			cbArgObj['Iam']={firstname:$('#firstName',this.response).val(),lastname:$('#lastName',this.response).val(),nickname:$('#nickName',this.response).val()};
			cb(cbArgObj);				
